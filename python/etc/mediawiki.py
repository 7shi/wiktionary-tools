...
class DB:
    def __init__(self, db):
        ...
        self.xmlbz2 = self.setting("xmlbz2")
        self.ns     = {}
        self.max    = self.cur.execute("SELECT MAX(id   ) FROM pages ").fetchone()[0]
        self.bmax   = self.cur.execute("SELECT MAX(block) FROM blocks").fetchone()[0]
    ...
    def index_block(self, title):
        result = self.cur.execute(
            "SELECT id, block FROM pages WHERE title = ?", (title,)).fetchone()
        return result if result else (None, None)

    @functools.lru_cache(maxsize=10000)
    def block_title(self, id):
        result = self.cur.execute(
            "SELECT block, title FROM pages WHERE id = ?", (id,)).fetchone()
        return result if result else (None, None)

    @functools.lru_cache(maxsize=1000)
    def pages(self, blockid):
        result = self.cur.execute(
            "SELECT start, length FROM blocks WHERE block = ?", (blockid,)).fetchone()
        ...
        xml = "<mediawiki>" + xml
        if blockid < self.bmax: xml += "</mediawiki>"
