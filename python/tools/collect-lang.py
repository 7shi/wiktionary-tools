...
def langtext(args):
    langs, names, target, pos, length = args
    ...
    for title, ns, id, rd, text in mediawiki_parse.getpages_xml(bz2.decompress(bz2data)):
        if ns != 0: continue
        for lang, text2 in mediawiki_parse.splittext(pattern, text):
            if (l := names.get(lang, None)):
                result[l].append((title, "".join(text2).strip()))
    return result
...
if __name__ == "__main__":
    ...
    with sqlite3.connect(db) as conn:
        ...
        lids, notfound, names = [], [], {}
        for lang in langs:
            if (result := cur.execute("SELECT lid FROM langname WHERE name = ?", (lang,)).fetchone()):
                lid = result[0]
                lids.append(lid)
                for (name,) in cur.execute("SELECT name FROM langname WHERE lid = ?", (lid,)):
                    names[name] = lang
            else:
        ...
        for sid, r in sorted(sids.items(), key=lambda x:x[0]):
            if cur != sid or length > 10_000_000:
                poslens.append((langs, names, target, pos, length))
                pos = r[1]
                length = 0
            length += r[2]
            cur = sid + 1
        if length: poslens.append((langs, names, target, pos, length))
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        for i, result in enumerate(executor.map(langtext, poslens), start=1):
            sys.stderr.write(f"\rreading streams... {i:,} / {len(poslens):,}")
            sys.stderr.flush()
            for lang in langs:
    ...
    for lang in langs:
        ld = langdata[lang]
        sys.stderr.write(f"{lang}: {len(ld):,}\n")
